...
from .utils import add_user_message, chat, Effort
...
class PromptEvaluator:
    def __init__(self, max_concurrent_tasks=3, output_dir="tmp"):
    ...
    def generate_unique_ideas(self, task_description, prompt_inputs_spec, num_cases, client, model):
        ...
        text = chat(
            ...
            model=model,
            effort=Effort.MAX,
            json_schema=ideas_schema,
        )

        return json.loads(text)

    def generate_test_case(self, task_description, idea, prompt_inputs_spec, client, model):
        ...
        prompt_inputs_properties = {key: {"type": "string"} for key in prompt_inputs_spec.keys()}
        test_case_schema = {
            "type": "object",
            "properties": {
                "prompt_inputs": {
                    "type": "object",
                    "properties": prompt_inputs_properties,
    ...
    def generate_dataset(
        ...
        output_file = output_file or self.output_dir / "dataset.json"

        ideas = self.generate_unique_ideas(
            task_description, prompt_inputs_spec, num_cases, client, model
        )
    ...
    def grade_output(self, test_case, output, extra_criteria, client, model):
    ...
    def run_test_case(self, test_case, run_prompt_function, client, model, extra_criteria=None):
        """Run a test case and grade the result"""
        output = run_prompt_function(test_case["prompt_inputs"])

        model_grade = self.grade_output(test_case, output, extra_criteria, client, model)
    ...
    def run_evaluation(
        ...
        dataset_file = dataset_file or self.output_dir / "dataset.json"
        json_output_file = json_output_file or self.output_dir / "output.json"
        html_output_file = html_output_file or self.output_dir / "output.html"
